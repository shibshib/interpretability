...
def test_fgsm( model, epsilon ):
    ...
    logger.info("Epsilon: {}\tTest Accuracy = {} / {} = {}".format(epsilon, correct, len(test_loader), final_acc))
...
def test_modified_accuracies():
    ...
    for e in epsilons:
        ...
        logger.info("Processing for e = {}".format(e))
        for m in models.keys():
            checkpoint_path = "./checkpoint/" + m + ".torch"
            checkpoint = torch.load(checkpoint_path, map_location=map_location)

            model = models[m].to(device)

            model.load_state_dict(checkpoint['model_state_dict'])

            logger.info("Loaded model {}.".format(m))
            acc, example = test_fgsm(model, e)
            logger.info("FGSM attack complete, epsilon={}, model={}, acc={}".format(e, m, acc))
            accuracies[m] = acc
            examples[m] = example
